...
public class FileDialog extends JDialog implements ActionListener {
    ...
    private void init() {
        ...
        if (dialog_type.equals("NEW")) {  // 工具栏中：新建
        	...
        	info_Action();            
        } else if(dialog_type.equals("INFO")){  // 工具栏中：文件信息
        	info_Action();             
        } else {  // 其他情况
            error_Action();
        }
    ...
    public void actionPerformed(ActionEvent e) {
    	...
    	try {
    		if (dialog_type.equals("NEW")) {  // 用户要新增一个文件
                ...
                frame.setMessage(m);
                frame.changeDiskInfo();
            } else if (dialog_type.equals("INFO")) {  // 显示文件的信息功能
               m = new Message(
            	...
            	int size = Disk.map.get(text_newName.getText()).getSize();
            	...
            	for(int i = 0;i<size;i++){
            		label_L[i] = new JLabel(" "+(i)+" ");
            		label_D[i] = new JLabel(" "+(start+i)+" ");
